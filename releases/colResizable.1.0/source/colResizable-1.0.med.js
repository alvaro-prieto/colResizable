...
(function($){ 	
	...
	A = ':absolute;',    
	ap = 'append',
	h = $("head")[ap](st+".CRZ{table-layout:fixed}.CRZ td,.CRZ th{overflow:hidden}.CRC{height:0px;"+U+":relative}.CRG{margin-left:-5px;"+U+A+" z-index:5}.CRG .CRZ{"+U+A+"background-color:red;filter:alpha(opacity=1);opacity:0;width:10px;cursor:e-resize;height:100%}.CRL{"+U+A+"width:1px}.CRD{border-left:1px dotted black"+se),			
	N = null,	
	D = N,			
	T = [],
	c = 0,	
	...
	function o( d, O){		
		...
		t.b  = I(k? d.border || d[c].borderLeftWidth :t.css(bo+'left-'+wi))||1;
		T[i] = t; 			
		G(t);		
	}
	...
	function G(t){	
		...
		h.each(function(i){									
			...
			c[wi](c.w)[r](wi);				
			if (i < t.l-1)	g.mousedown(w).html('<div class="'+S+'"></div>')[ap](t.o.gripInnerHtml);  	
	...
	function g(t){	
		...
		for(var c,i=0; i<t.l; i++){	
			c = t.c[i]; 			
			t.g[i].css({			
				left: c.offset().left - t.offset().left + c.outerWidth() + t.s / 2 + p,
	...
	*/
	...
	function m(e){	
		...
		w = 20, i = D.i,		 
		l = t.s*1.5 + w + t.b,
		G = i == t.l-1? t.w-l: t.g[i+1][U]().left-t.s-w, 
		o = i? t.g[i-1][U]().left+t.s+w: l;				 
		x = M.max(o, M.min(G, x));							
		D.x = x;	 D.css("left",  x + p); 						
		if(t.o.liveDrag){ C(t,i);g(t);}					
		return F; 					
	}
	...
	function w(e){
		...
		g.L = e.pageX;	g.l = g[U]().left;	 				
		d.bind(mm+S, m).bind(mu+S,O);	 
		h[ap](st+"*{cursor:e-resize!important"+se); 		 
