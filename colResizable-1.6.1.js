/**
	...
	v1.6.1 - jQuery plugin created by Alvaro Prieto Lauroba
...
(function ($) {
	...
	var init = function (tb, options) {
		...
		if (t.opt.rtl) h.append("<style type='text/css'>.rtl .JCLRgrip{ margin-left:0; margin-right:-5px; }</style>"); //3i2/ if rtl has been set, append the style
	...
	var syncGrips = function (t) {
		...
		for (var i = 0; i < t.ln; i++) { //for each column
			...
			t.g[i].css({ //height and position of the grip is updated according to the table layout
				left: t.opt.rtl ? c.offset().left - t.offset().left + /*c.outerWidth(false) +*/ t.cs / 2 + PX : c.offset().left - t.offset().left + c.outerWidth(false) + t.cs / 2 + PX, //3i2
	...
	var onGripDrag = function (e) {
		...
		if (t.opt.rtl) {
			//3i2
			min = last ? t.f ? t.position().left + t.cs + mw : 0 : t.g[i + 1].position().left + t.cs + mw;
			max = i ? t.g[i - 1].position().left - t.cs - mw : t.c[i].position().left + t.c[i].width() - t.cs - mw;
		} else {
			min = i ? t.g[i - 1].position().left + t.cs + mw : l; //min position according to the contiguous cells
			max = t.f ? //fixed mode?
				i == t.ln - 1 ?
				t.w - l :
	...
	$.fn.extend({
		colResizable: function (options) {
			var defaults = {
				...
				rtl: false //3i2/ for Right-To-Left Tables
			}
