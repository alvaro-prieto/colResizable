...
(function ($) {
	...
	var init = function (tb, options) {
		...
		if (table.opt.hoverCursor !== 'col-resize') {
		...
		table.w = table.width();
		table.gc = table.prev();
		table.fixed = table.opt.fixed;

		if (options.marginLeft) {
			//if the table contains margins, it must be specified
			table.gc.css("marginLeft", options.marginLeft);
		}

		if (options.marginRight) {
			//since there is no (direct) way to obtain margin values in its original units (%, em, ...)
			table.gc.css("marginRight", options.marginRight);
		}
		//table cellspacing (not even jQuery is fully cross-browser)
		table.cs = I(ie ? tb.cellSpacing || tb.currentStyle.borderSpacing : table.css('border-spacing')) || 2;
		//outer border width (again cross-browser issues)
		table.b = I(ie ? tb.border || tb.currentStyle.borderLeftWidth : table.css('border-left-width')) || 1;
	...
	var createGrips = function (table) {
		...
		th.each(function (i) {						//iterate through the table column headers
			...
			var disabledColumns = table.disabledColumns.indexOf(i) != -1;           //is this a disabled column?
			var grip = $(table.gc.append('<div class="JCLRgrip"></div>')[0].lastChild); //add the visual node to be used as grip
			grip.append(disabledColumns ? "" : table.opt.gripInnerHtml).append('<div class="' + SIGNATURE + '"></div>');
			...
			if (i == table.ln - 1) {
				...
				//if the table resizing mode is set to fixed, the last grip is removed since table with can not change
				if (table.fixed) {
					grip.html("");
				}
			}
			...
			grip.data(SIGNATURE, { i: i, t: table.attr(ID), last: i == table.ln - 1 });	 //grip index and its table name are stored in the HTML
		});
	...
	*/
	var syncCols = function (table, i, isOver) {
		var inc = drag.x - drag.l;
		// retrieve dragged and next column
		var draggedCol = table.c[i];
		var nextCol = table.c[i + 1];
		//their new width is obtained
		var width = draggedCol.w + inc;
		var width2 = nextCol.w - inc;
		draggedCol.width(width + PX);
		table.cg.eq(i).width(width + PX);

		if (table.fixed) { //if fixed mode
			nextCol.width(width2 + PX);
			table.cg.eq(i + 1).width(width2 + PX);
	...
	*/
	...
	var onGripDrag = function (e) {
		...
		var mw = table.opt.minWidth; //cell's min width
		var i = drag.i;
		var l = table.cs * 1.5 + mw + table.b;
		var last = i == table.ln - 1;                 			//check if it is the last column's grip (usually hidden)
		var min = i ? table.g[i - 1].position().left + table.cs + mw : l;	//min position according to the contiguous cells
		var max = table.fixed ? 	//fixed mode?
			i == table.ln - 1 ?
				table.w - l :
				table.g[i + 1].position().left - table.cs - mw :
		...
		if (last) {
			var c = table.c[drag.i]; //width of the last column is obtained
			drag.w = c.w + x - drag.l;
		}
	...
	var onGripDragOver = function (e) {
		...
		if (!(drag.x - drag.l == 0)) {
			...
			var last = i == table.ln - 1;         //check if it is the last column's grip (usually hidden)
			var draggedCol = table.g[i].c;  //the column being dragged
			...
			syncGrips(table);				//the grips are updated
